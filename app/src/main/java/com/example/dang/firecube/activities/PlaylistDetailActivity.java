package com.example.dang.firecube.activities;
...
import com.example.dang.firecube.adapters.SongsListAdapter;
import com.example.dang.firecube.dataloaders.LastAddedLoader;
...
import com.example.dang.firecube.dataloaders.TopTracksLoader;
...
import com.example.dang.firecube.widgets.DragSortRecycler;
...
public class PlaylistDetailActivity extends BaseActivity implements ATEActivityThemeCustomizer, ATEToolbarCustomizer {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (animate && FirecubeUtils.isLollipop() && PreferencesUtility.getInstance(this).getAnimations()) {
    ...
    private void setAlbumart() {
        ...
        loadBitmap(FirecubeUtils.getAlbumArtUri(getIntent().getExtras().getLong(Constants.ALBUM_ID)).toString());
    }
    ...
    private void setRecyclerViewAapter() {
        ...
        if (animate && FirecubeUtils.isLollipop() && PreferencesUtility.getInstance(mContext).getAnimations()) {
    ...
    private void clearAutoPlaylists() {
        switch (action) {
            case Constants.NAVIGATE_PLAYLIST_LASTADDED:
                FirecubeUtils.clearLastAdded(this);
                break;
            case Constants.NAVIGATE_PLAYLIST_RECENT:
                FirecubeUtils.clearRecent(this);
                break;
            case Constants.NAVIGATE_PLAYLIST_TOPTRACKS:
                FirecubeUtils.clearTopTracks(this);
                break;
        }
        Intent returnIntent = new Intent();
        setResult(Activity.RESULT_OK, returnIntent);
