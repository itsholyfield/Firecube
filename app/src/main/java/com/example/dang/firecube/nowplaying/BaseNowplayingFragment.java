...
import com.example.dang.firecube.activities.BaseActivity;
import com.example.dang.firecube.adapters.BaseQueueAdapter;
import com.example.dang.firecube.adapters.SlidingQueueAdapter;
import com.example.dang.firecube.dataloaders.QueueLoader;
import com.example.dang.firecube.listeners.MusicStateListener;
import com.example.dang.firecube.timely.TimelyView;
...
import com.example.dang.firecube.utils.SlideTrackSwitcher;
import com.example.dang.firecube.utils.FirecubeUtils;
import com.example.dang.firecube.widgets.CircularSeekBar;
import com.example.dang.firecube.widgets.DividerItemDecoration;
import com.example.dang.firecube.widgets.PlayPauseButton;
import com.example.dang.firecube.widgets.PlayPauseDrawable;
...
public class BaseNowplayingFragment extends Fragment implements MusicStateListener {
        ...
        public void run() {
            ...
            if (mProgress != null) {
                ...
                if (elapsedtime != null && getActivity() != null)
                    elapsedtime.setText(FirecubeUtils.makeShortTimeString(getActivity(), position / 1000));
            }
            overflowcounter--;
    ...
    public Runnable mUpdateElapsedTime = new Runnable() {
        ...
        public void run() {
            if (getActivity() != null) {
                String time = FirecubeUtils.makeShortTimeString(getActivity(), MusicPlayer.position() / 1000);
                if (time.length() < 5) {
    ...
    public void setSongDetails(View view) {
        ...
        if (playPauseFloating != null) {
            playPauseDrawable.setColorFilter(FirecubeUtils.getBlackWhiteColor(accentColor), PorterDuff.Mode.MULTIPLY);
    ...
    public void updateSongDetails() {
        ...
        if (!duetoplaypause) {
            if (albumart != null) {
                ImageLoader.getInstance().displayImage(FirecubeUtils.getAlbumArtUri(MusicPlayer.getCurrentAlbumId()).toString(), albumart,
        ...
        if (songduration != null && getActivity() != null)
            songduration.setText(FirecubeUtils.makeShortTimeString(getActivity(), MusicPlayer.duration() / 1000));
